...
class NotificationParserEnhancer:
    ...
    def parse(self,text: str)->ParsedNotification:
        ...
        m=re.search(r"(?:\b\w[\w -]*\s+)?vacancies?\s+for\s+the\s+post\s+of\s+([^\n.]+)",normalized,re.I)
        if not title.value and m: title=ParsedField(m.group(1).strip(),m.group(0),"high")
        m=re.search(r"\b(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|one hundred)\s+vacancies?\s+for\s+the\s+post",normalized,re.I)
