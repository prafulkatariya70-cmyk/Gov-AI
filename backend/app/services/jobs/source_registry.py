...
class SourceRegistryService:
    def __init__(self, db: Session, *, discovery=None):
        self.db=db; self.discovery=discovery or OfficialSourceDiscovery()
    def due_sources(self):
        now=datetime.now(timezone.utc); rows=self.db.query(JobSourceRegistry).filter(JobSourceRegistry.active.is_(True)).all()
    ...
    def check(self, source):
        ...
        try:
            discovered=self.discovery.discover(source.listing_url); source.last_success_at=datetime.now(timezone.utc); self.db.commit(); return discovered
        except Exception:
